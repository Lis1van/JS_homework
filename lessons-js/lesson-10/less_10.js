...
function displayObject() {
    ...
    objectContainer.innerHTML = `<pre>${JSON.stringify(person, null, 2)}</pre>`;
}
...
function updateCount() {
    let count = localStorage.getItem('pageReloadCount') || 0;
    count++;
    localStorage.setItem('pageReloadCount', count);
    const countContainer = document.getElementById('count-container');
    countContainer.innerHTML = `Кількість перезавантажень: ${count}`;
}
